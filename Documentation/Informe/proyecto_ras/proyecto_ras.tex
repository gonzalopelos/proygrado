...
Para el desarrollo del firmware se utilizó la plataforma Kinetis Design Studio, que es un entorno de desarrollo para Kinetis MCU y permite la edición robusta, la compilación y la depuración de sus diseños. Está basado en software libre y de código abierto que incluye Eclipse, GNU Compiler Collection (GCC), GNU Debugger (GDB).
También se utlizaron las siguientes librerías de código en C++:
\begin{itemize}
	\item \textit{EthernetInterface}: librería que provee una API para establecer conexiones a internet. En particular se utilizó esta librería para crear sockets TCP y establecer la comunicación con la SBC.
	\item \textit{mbed-rtos}: esta librería ofrece la implementación del Sistema operativo oficial mbed en tiempo real basado en la implementación RTX del estándar abierto de la API CMSIS-RTOS.
	\item \textit{mpc4728}: librería que implmenta las interfaces I2CInterface y DACInterface, provistas por mbed, para proveer una API que permite la comunicación con el dispositivo DAC MCP4728.
	\item \textit{mbed}: SDK oficial de mbed C / C ++ que proporciona la plataforma de software y las bibliotecas básicas para el desarrollo de aplicaciones.
\end{itemize}
...
En la bibliografía relevada se proponen nuevas soluciones que abordan varios aspectos de la seguridad. Sin embargo los estándares son dejados de lado, se favorece en cambio aproximaciones no estandarizadas al desarrollo de robots seguros. No obstante, las escaza utilización de estandares limita la credibilidad acerca de la seguridad alcanzada, y limita la usabilidad de dichos resultados. Una razón para esto es que el estándar ISO 13482, que aplica a algunos tipos de field robots, es aun nuevo. 

A pesar de no contar con un estándar esLa falta un estándar específico que aplique a un robot con las características del DM3, hizo teniendo que reunir información de varios ens

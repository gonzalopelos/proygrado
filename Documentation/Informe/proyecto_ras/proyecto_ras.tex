...
%%
\abstract{%
  %“Within two decades, all farms will have some sort of robot”, David Gardner, jefe ejecutivo de la Royal Agricultural Society Of England (Dentro de las próximas dos décadas, todas las granjas tendrán alguna clase de robot)
  
  El presente documento es un registro del trabajo realizado en el marco del proyecto de grado RAS. Se espera que con la lectura de éste el lector pueda comprender los problemas asociados a la garantizar la seguridad en la operación de un robot autónomo y colaborativo que asiste a humanos en tareas de recolección de frutos.
  
  Se describe el desarrollo de los objetivos del proyecto los cuales eran, la revisión del estado del arte de la seguridad en la robótica colaborativa y autónoma haciendo énfasis en las aproximaciones estandarizadas a este ámbito de conocimiento, la confección de una guía de recomendaciones de seguridad nutrida de los preceptos encontrados durante esta revisión y por último la implementación e integración al robot prototipo DM3 de un conjunto mecanismos de seguridad.
  
  Entre las principales contribuciones de este proyecto se encuentra, un revelamiento de las normas y estándares internacionales (así como trabajo de terceros) sobre seguridad aplicables a un robot autónomo y colaborativo para el agro condensados en una guía de seguridad, que podrá ser considerada como referencia para futuros trabajos y una arquitectura y solución de software flexible y fácilmente extensible que facilita la integración de nuevos mecanismos de seguridad.
  
  Analizados los resultados experimentales obtenidos, se puede concluir que la solución aquí propuesta aumenta sustancialmente la seguridad del prototipo DM3 en su operativa, no sólo para humanos y el ambiente, sino también para el robot mismo.
}
...
\newglossaryentry{SBC}{
	name=SBC,
	description={en inglés \textbf{S}ingle \textbf{B}oard \textbf{C}omputer, es una computadora completa construida en una sola placa de circuito}
}

\newglossaryentry{DAC}{
	name=DAC,
	description={en inglés \textbf{D}igital to \textbf{A}nalogue \textbf{C}onverter, es un dispositivo conversor de señal digital a analógica}
}
\newglossaryentry{I2C}{
	name=I2C,
	description={en inglés \textbf{I}nter-\textbf{I}ntegrated \textbf{C}ircuit, es un protocolo en serie para conectar dispositivos de baja velocidad en un circuito integrado}
}

\newglossaryentry{Socket}{
	name=Socket,
	description={designa un concepto abstracto por el cual dos programas (probablemente residentes en computadoras distintas) pueden intercambiar un flujo de datos}
}

\newglossaryentry{USB}{
	name=USB,
	description={en inglés \textbf{U}niversal \textbf{S}erial \textbf{B}us, es un bus estándar industrial que define los conectores y protocolos utilizados  para conectar, comunicar y proveer de alimentación eléctrica a dispositivos electrónicos}
}

\newglossaryentry{Ethernet}{
	name=Ethernet,
	description={es una familia de tecnologías de redes informáticas}
}

\newglossaryentry{conectorRJ45}{
	name=conector RJ45,
	description={es una interfaz física comúnmente utilizada para conectar redes de computadoras con cableado estructurado}
}

\newglossaryentry{Bumper}{
	name=Bumper,
	description={sensor de contacto de uso común en sistemas robóticos para la detección de colisiones}
}

\newglossaryentry{GPS}{
	name=GPS,
	description={en inglés \textbf{G}lobal \textbf{P}ositioning \textbf{S}ystem, es un sistema que permite determinar, en toda la Tierra, la posición de un objeto}
}
...
\item  \textul{Communication}, para monitorear el estado de la conexión entre los sistemas en las placas\gls{SBC} y de entrada/salida.
...
En esta sección se presentan las pruebas realizadas para evaluar el desarrollo de los mecanismos de seguridad sobre un prototipo experimental. En la Sección 7.1 se describen la pruebas funcionales realizadas y en la Sección 7.2 las pruebas de rendimiento, finalmente, en la Sección 7.3 se presenta la evaluación de los resultados obtenidos.

	...
	char* str;
 	...
 	switch(command){
		...
		case 2:
			...
			unsigned char data_exa[256];
			unsigned int frame_length;
			frame_data("test", data_exa, &frame_length);

			res = hid_write(usb, data_exa, frame_length);
			...
			if (res < 0){
				...
				while (res == 0) {
					res = hid_read(usb, data_read, frame_length);
					if (res == 0) {
						printf("waiting...\n");
					}
					else if (res < 0){
						...
						hexToString(frame, data_read, res);
						get_data(frame, frame_length, data, &data_length);
...
int frame_data(const char * send_data, unsigned char* frame_data_hexa, unsigned int* frame_length){
	int ret;
	...
	*frame_length = 0;
	...
	if (ret == 0)
	{
		...
		stringToHex(frame_data_hexa, frame_data, *frame_length);
